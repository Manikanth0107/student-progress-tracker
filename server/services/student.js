...
const fetchAndUpdateRatings = async (student) => {
  try {
    ...
    if (contests.length > 0) {
      ...
      await student.save();
      console.log(
        `Updated ratings for ${student.codeforcesHandle}: current=${student.currentRating}, max=${student.maxRating}`
      );
    }
    return student;
  } catch (error) {
    console.error(
      `Error updating ratings for ${student.codeforcesHandle}:`,
      error
    );
    return student;
  }
...
const updateStudent = async (id, updates) => {
  ...
  if (updates.codeforcesHandle) {
    ...
    const existingStudent = await Student.findOne({
      codeforcesHandle: updates.codeforcesHandle,
      _id: { $ne: id },
    });
    if (existingStudent) {
...
const saveContestUnsolvedCount = async (handle, contestId, unsolvedCount) => {
  await Student.updateOne(
    ...
    {
      $set: {
        "contests.$.unsolvedCount": unsolvedCount,
      },
    }
  );
};
